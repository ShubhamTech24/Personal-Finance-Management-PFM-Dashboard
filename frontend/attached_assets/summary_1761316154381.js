...
const basePath = PLAID_ENV === 'sandbox' ? PlaidEnvironments.Sandbox : PlaidEnvironments.Development;
const client = new PlaidApi(new Configuration({ basePath, baseOptions: { headers: { 'PLAID-CLIENT-ID': PLAID_CLIENT_ID, 'PLAID-SECRET': PLAID_SECRET } } }));
...
router.get('/spend-by-category', auth, async (req, res) => {
  try {
    ...
    if (!acc) return res.status(404).json({ message: 'No linked account' });
...
router.get('/monthly-summary', auth, async (req, res) => {
  try {
    ...
    const end = new Date();
    const start = new Date(); start.setMonth(end.getMonth() - 5);
